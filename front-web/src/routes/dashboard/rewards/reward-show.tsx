...
import type { RewardShowData } from "../../../types/ShowType";

export async function clientLoader({
  params,
}: {
  params: { id: string };
}): Promise<RewardShowData | { error: string }> {
  try {
    const { id } = params;
...
export default function RewardShow({
  ...
  loaderData: RewardShowData | { error: string };
}) {
  const { t } = useTranslation(["show", "navigation", "common"]);
  const { lang } = useParams();

  const data = "code" in loaderData ? { ...loaderData } : loaderData;
  ...
  return (
    <div className="container">
      ...
      <main className="main-content">
        ...
        <Show
          title={t("titleDetails.reward", { ns: "show", defaultValue: "Reward Details" })}
          data={data}
          backUrl={`/${lang}/dashboard/rewards`}
          editUrl={"id" in data ? `/${lang}/dashboard/rewards/${data.id}/edit` : undefined}
        />
      </main>
    </div>
  );
