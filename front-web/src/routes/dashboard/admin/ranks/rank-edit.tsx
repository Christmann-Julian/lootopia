...
import type { EditRankFormData } from "../../../../types/FormType";
...
export async function clientAction({ request }: ClientActionFunctionArgs) {
  ...
  try {
    await api.put(`/api/ranks/${data.id}`, data);
    return { success: true };
  } catch (err: unknown) {
...
export async function clientLoader({
  ...
  params: { id: string };
}): Promise<EditRankFormData | { error: string }> {
  try {
    const { id } = params;
...
export default function RankEdit({
  ...
  loaderData: EditRankFormData | { error: string };
}) {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<EditRankFormData>({
    defaultValues: "error" in loaderData ? undefined : loaderData,
  });

  const onSubmit: SubmitHandler<EditRankFormData> = (data: EditRankFormData) => {
    const dataWithId = {
      ...data,
  ...
  useEffect(() => {
    if (fetcher.data?.success) {
      setToast({
        message: t("toast.rankUpdated", { ns: "form" }),
        type: "success",
      });
        ...
        <form className="form-container" onSubmit={handleSubmit(onSubmit)}>
          <div className="card">
            <div className="card-header">
              <h2 className="card-title">{t("title.editRank", { ns: "form" })}</h2>
              <p className="card-description">{t("description.editRank", { ns: "form" })}</p>
            </div>
