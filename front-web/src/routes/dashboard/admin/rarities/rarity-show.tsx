...
import { type RarityShowData } from "../../../../types/ShowType";

export async function clientLoader({
  params,
}: {
  params: { id: string };
}): Promise<RarityShowData | { error: string }> {
  try {
    const { id } = params;
...
export default function RarityShow({
  loaderData,
}: {
  loaderData: RarityShowData | { error: string };
}) {
  const { t } = useTranslation(["show", "navigation", "common"]);
  const { lang } = useParams();

  const data = "minExperience" in loaderData ? { ...loaderData } : loaderData;
  ...
  return (
    <div className="container">
      ...
      <main className="main-content">
        ...
        <Show
          title={t("titleDetails.rarity", { ns: "show", defaultValue: "Rarity Details" })}
          data={data}
          backUrl={`/${lang}/dashboard/admin/rarities`}
          editUrl={"id" in data ? `/${lang}/dashboard/admin/rarities/${data.id}/edit` : undefined}
        />
      </main>
    </div>
  );
