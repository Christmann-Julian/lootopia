...
import type { EditRarityFormData } from "../../../../types/FormType";
...
export async function clientAction({ request }: ClientActionFunctionArgs) {
  ...
  try {
    await api.put(`/api/rarities/${data.id}`, data);
    return { success: true };
  } catch (err: unknown) {
...
export async function clientLoader({
  ...
  params: { id: string };
}): Promise<EditRarityFormData | { error: string }> {
  try {
    const { id } = params;
    const response = await api.get(`/api/rarities/${id}`, {
      headers: {
        "X-Skip-Locale": "true",
      },
    });
...
export default function RarityEdit({
  ...
  loaderData: EditRarityFormData | { error: string };
}) {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<EditRarityFormData>({
    defaultValues: "error" in loaderData ? undefined : loaderData,
  });

  const onSubmit: SubmitHandler<EditRarityFormData> = (data: EditRarityFormData) => {
    const dataWithId = {
      ...data,
  ...
  useEffect(() => {
    if (fetcher.data?.success) {
      setToast({
        message: t("toast.rarityUpdated", { ns: "form" }),
        type: "success",
      });
        ...
        <form className="form-container" onSubmit={handleSubmit(onSubmit)}>
          <div className="card">
            <div className="card-header">
              <h2 className="card-title">{t("title.editRarity", { ns: "form" })}</h2>
              <p className="card-description">{t("description.editRarity", { ns: "form" })}</p>
            </div>
