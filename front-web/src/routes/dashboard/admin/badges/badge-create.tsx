...
import type { CreateBadgeFormData } from "../../../../types/FormType";

export async function clientAction({ request }: ClientActionFunctionArgs) {
  const data = await request.json();

  try {
    await api.post(`/api/badges`, data);
    return { success: true };
  } catch (err: unknown) {
...
export default function BadgeCreate() {
  ...
  const onSubmit: SubmitHandler<CreateBadgeFormData> = (data: CreateBadgeFormData) => {
  ...
  useEffect(() => {
    if (fetcher.data?.success) {
      setToast({
        message: t("toast.badgeCreated", { ns: "form" }),
        type: "success",
      });
      reset();
        ...
        <form className="form-container" onSubmit={handleSubmit(onSubmit)}>
          <div className="card">
            <div className="card-header">
              <h2 className="card-title">{t("title.createBadge", { ns: "form" })}</h2>
              <p className="card-description">{t("description.createBadge", { ns: "form" })}</p>
            </div>
