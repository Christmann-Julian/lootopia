...
import type { EditCategoryFormData } from "../../../../types/FormType";
...
export async function clientLoader({
  ...
  params: { id: string };
}): Promise<EditCategoryFormData | { error: string }> {
  try {
    const { id } = params;
...
export default function CategoryEdit({
  ...
  loaderData: EditCategoryFormData | { error: string };
}) {
  ...
  const onSubmit: SubmitHandler<EditCategoryFormData> = (data: EditCategoryFormData) => {
    const dataWithId = {
      ...data,
  ...
  useEffect(() => {
    if (fetcher.data?.success) {
      setToast({
        message: t("toast.categoryUpdated", { ns: "form" }),
        type: "success",
      });
        ...
        <form className="form-container" onSubmit={handleSubmit(onSubmit)}>
          <div className="card">
            <div className="card-header">
              <h2 className="card-title">{t("title.editCategory", { ns: "form" })}</h2>
              <p className="card-description">{t("description.editCategory", { ns: "form" })}</p>
            </div>
